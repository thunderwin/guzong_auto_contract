...
import { buildCheckMessages } from "@/lib/contract/prompts";
import { checkRequestSchema, contractCheckSchema } from "@/lib/contract/schemas";
...
export async function POST(request: Request) {
  ...
  const parsedRequest = checkRequestSchema.safeParse(body);
  ...
  try {
    const { system, user } = buildCheckMessages(parsedRequest.data);
    ...
    const parsedResult = contractCheckSchema.safeParse(json);
    ...
    const message =
      error instanceof Error ? error.message : "合同检查失败，请稍后重试";
